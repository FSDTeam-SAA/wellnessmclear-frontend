 ...
 * Provides methods for user management, messaging, groups, and payments
 */
class ApiService {
  /**
  ...
  /**
   * Fetch method for FormData requests (file uploads)
   ...
   * @param fallback - Fallback value if data extraction fails
  ...
  async getMessages(
    ...
    const response = await this.fetchWithAuth(
      `/message/${groupId}?page=${page}&limit=${limit}`, 
      {}, 
      token
    )
  ...
  /**
   * Send a new message to a group (supports file uploads)
   * @param groupId - Target group identifier
   * @param formData - Message data (text, files, etc.)
   * @param token - Authentication token
  ...
  async editMessage(messageId: string, content: string, token: string): Promise<Message> {
    return await this.fetchWithAuth(`/message/${messageId}`, {
      method: 'PUT',
  ...
  async deleteMessage(messageId: string, token: string): Promise<void> {
    await this.fetchWithAuth(`/message/${messageId}`, {
      method: 'DELETE',
    }, token)
  }
  ...
  /**
   * Join a group
   * @param groupId - Group identifier to join
   * @param token - Authentication token
   * @returns Promise resolving to void
   */
  async joinGroup(groupId: string, token: string): Promise<void> {
    await this.fetchWithAuth(`/group/join/${groupId}`, {
      method: 'POST',
    }, token)
  }
   ...
   * @param token - Authentication token
   * @returns Promise resolving to payment result with optional checkout URL
   */
  async processPayment(
    data: PaymentRequest, 
    token: string
  ): Promise<{ success: boolean; checkoutUrl?: string }> {
    return await this.fetchWithAuth('/payment/checkout', {
      method: 'POST',
